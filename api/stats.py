...
def exp_stats(exp_dict, schedule):
    ...
    plt.bar(
        list(slot_exp_dict.keys()),
        slot_exp_dict.values(),
        color="b",
        align="edge",
        width=0.3,
    )
...
def skill_stats(skill_dict, schedule):
    ...
    plt.figure(figsize=(15, 3))
    plt.bar(
        list(slot_skill_dict.keys()),
        slot_skill_dict.values(),
        color="b",
        align="edge",
        width=0.3,
    )
        ...
        for student in schedule[slot]:
            ...
            if slot_type == constants.FOUR_HOUR_SLOT:
                ...
                for i_slot in prev_slot.keys():
                    if (
                        student in schedule[i_slot]
                        and student in schedule[prev_slot[i_slot]]
                    ):
